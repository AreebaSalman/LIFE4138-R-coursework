...
deseq2_data1 <- read_tsv("C:/Users/HP/Desktop/R Studio/Computational Assignment/A_vs_F.deseq2.results.tsv")
...
missing_data_summary <- sum(is.na(deseq2_data1))
cat("Number of missing values:", missing_data_summary, "\n")

# Remove rows with missing values (if any)
deseq2_data1 <- na.omit(deseq2_data)
...
# MA plot
ggplot(deseq2_data1, aes(x = baseMean, y = log2FoldChange, color = padj < pvalue_threshold)) +
...
ggplot(deseq2_data1, aes(x = padj)) +
  geom_histogram(bins = 50, fill = "blue", color = "black", alpha = 0.8) +
  labs(title = "Distribution of Adjusted P-Values", x = "Adjusted P-Value", y = "Frequency") +
  theme_minimal()

# Save the plot
ggsave("pvalue_histogram1.png")
...
ggplot(deseq2_data1, aes(x = log2FoldChange, y = -log10(padj), color = Significance)) +
  geom_point(alpha = 0.7, size = 2) +
  scale_color_manual(values = c("Upregulated" = "blue", "Downregulated" = "red", "Not Significant" = "yellow")) +
...
# Save the plot
ggsave("volcano_plot_ggplot1.png", width = 8, height = 6)


# Prepare data for heatmap
# Select the top 20 genes based on significance
top_genes <- deseq2_data1 %>%
  arrange(padj) %>%
  slice_head(n = 20) %>%
...
ggplot(heatmap_data, aes(x = gene, y = factor(row_id), fill = log2FoldChange)) +
  ...
  scale_fill_gradient2(low = "green", mid = "grey", high = "yellow", midpoint = 0) +
...
# Save the plot
ggsave("heatmap_ggplot1.png", width = 8, height = 6)

# Create lists of significant genes
significant_genes1 <- deseq2_data1 %>%
  filter(padj < pvalue_threshold & abs(log2FoldChange) > logfc_threshold) %>%
  arrange(desc(log2FoldChange))
significant_genes1

# Save to CSV
write_csv(significant_genes, "significant_genes1.csv")
